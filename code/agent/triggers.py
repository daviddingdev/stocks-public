...
data/trigger_state.json:
  1. Held name moves >= threshold (JPM: price_move_pct_holdings; agent: agent_action_move_pct,
...
def held_symbols():
    jpm = [t for t, m in _j(CONF / "positions.json", {}).items() if (m.get("shares") or 0) > 0]
...
def alert(state, c, key, kind, symbol, msg, action=False, book=""):
    ...
    (JPM / Agent / both) and quantifies portfolio impact in the message."""
...
def run():
    ...
    def impact(tk, price, prev):
        ...
        if tk in jpm_pos:
            ...
            parts.append(f"JPM ${v / 1000:,.1f}k{w}, day {'-' if day < 0 else '+'}${abs(day):,.0f}")
            books.append("JPM")
        if tk in ag_pos:
    ...
    # feed.json is agent-universe only since 2026-08-12 (book independence) — JPM
    ...
    print(f"{dt.datetime.now(dt.timezone.utc).isoformat(timespec='seconds')} triggers: {fired} fired "
          f"({len(jpm_pos)} jpm, {len(ag_pos)} agent holdings watched)")
